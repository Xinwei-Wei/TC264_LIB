...
void IIC1_Start(void)
{
	...
	iic_delay();
	iic_delay();
 	IIC_SDA1_L; //START:when CLK is high,DATA change form high to low
	iic_delay();
	iic_delay();
	iic_delay();
	IIC_SCL1_L; //钳住I2C总线，准备发送或接收数据
}
...
void IIC1_Stop(void)
{
	SDA1_OUT; //sda线输出
	IIC_SCL1_L;
	IIC_SDA1_L; //STOP:when CLK is high DATA change form low to high
    iic_delay();
	iic_delay();
	iic_delay();
	IIC_SCL1_H;
...
uint8 IIC1_WaitAck(void)
{
	uint8  ucErrTime=0;
	SDA1_IN; //SDA设置为输入  （从机给一个低电平做为应答）
	IIC_SDA1_H;iic_delay();
	IIC_SCL1_H;iic_delay();
	while(IIC_SDA1_READ)
	{
		ucErrTime++;
		if(ucErrTime>100)
...
void IIC1_SendByte(uint8 data_t)
{
    uint8  t;
    SDA1_OUT;
    IIC_SCL1_L; //拉低时钟开始数据传输
    for(t=0;t<8;t++)
    {
//        IIC_SDA_READ = data_t&0x80;
        if(data_t&0x80)
...
uint8 IIC1_ReadByte(uint8 ack)
{
    ...
    for(i=0;i<8;i++ )
	{
        ...
        IIC_SCL1_H;
        receive<<=1;
        if(IIC_SDA1_READ)receive++; //从机发送的电平
        iic_delay();
    }
    if(ack)
        IIC1_Ack(); //发送ACK
